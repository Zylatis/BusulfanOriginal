...
model<-1            # choice of lambda model
p<-1/2              # power related to initial distribution of host cells
...
export.folder<-"ROutputs/AgeDependentLoss/"
...
findFit<-function(transformed.data.vector,nuVal,muVal){
  ...
  upper<-c(1,1,5)
  start<-c(0.1,0.01,0.01)
  
  out <- GenSA(par=start,lower = lower, upper = upper, fn = cost.function, control=list(verbose=TRUE,temperature=temp,maxit=maxit),transformed.data.vector,nuVal,muVal)
  fitPars<-out["par"][[1]]
  return(fitPars)
...
cost.function<-function(inVec,transformed.data,nuVal,muVal){
  pars<-c(inVec,nuVal,muVal) #add back my and nu - need to be in header in correct order!
  names(pars)<-header
...
make.plot<-function(inVec,nuVal,muVal){
  pars<-c(inVec,nuVal,muVal) #add back nu and mu - need to be in header in correct order!
  names(pars)<-header
  pars<-as.data.frame(t(pars))
...
fit.thymic.parameters<-function(precursor.counts,donor.precursor.dp1){
  ...
  muVal<-coef(mu.fit)[[2]]
  return(list(nuVal,muVal,predict(nu.fit),predict(mu.fit)))
}
########################################################################################################
...
if(!file.exists(best.file)){
  ...
  best.fit.pars<-c(findFit(transformed.data,nu.val,mu.val),nu.val,mu.val)
  ...
  best.SSR<-data.frame(SSR=cost.function(as.numeric(best.fit.pars),transformed.data,nu.val,mu.val))
...
doBoot<-function(pairedMice,i){
  ...
  output<-c(findFit(new.data,new.nu.val,new.mu.val),new.nu.val,new.mu.val)
